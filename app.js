...
document.getElementById('generatePlanBtn')?.addEventListener('click', async () => {
    ...
    const prompt = "Задание на практику: " + practiceTask + "\n";
    const prompt2 = "Общее количество часов: " + targetHoursTotal + "\n";
    const prompt3 = "Дата начала: " + startDate + "\n\n";
    const prompt4 = "Составь план выполнения задания, разбитый по дням. Укажи для каждого дня:\n";
    const prompt5 = "- День (номер или дата)\n- Количество часов\n- Что нужно сделать\n\n";
    const prompt6 = "План должен быть реалистичным. Оформи план в виде HTML с тегами ul, li, strong.";
    
    const fullPrompt = prompt + prompt2 + prompt3 + prompt4 + prompt5 + prompt6;
    
    document.getElementById('planResult').style.display = 'block';
    document.getElementById('planContent').innerHTML = '<p>⏳ Генерация плана...</p>';
    
    try {
// 🔧 Обновляем модель на gemini-3.5-flash
        ...
        let planContent = result.candidates[0].content.parts[0].text;
...
document.getElementById('generateReportBtn')?.addEventListener('click', async () => {
    ...
    let prompt = "Создай официальный отчет по производственной практике.\n\n";
    prompt += "Информация о студенте:\n";
    prompt += "ФИО: " + userName + "\n";
    prompt += "Место практики: " + practicePlace + "\n";
    prompt += "Задание на практику: " + practiceTask + "\n";
    prompt += "Общее количество часов: " + totalHoursSum + "\n\n";
    prompt += "Ежедневные записи студента:\n" + diaryText + "\n";
    
    if (exampleText) {
        prompt += "Ниже представлен пример отчета. Следуй его структуре и стилю оформления:\n\n";
        prompt += "ПРИМЕР:\n" + exampleText + "\n\n";
    }
    
    prompt += "Требования к отчету:\n";
    prompt += "1. Используй официально-деловой стиль\n";
    prompt += "2. Укажи место практики: " + practicePlace + "\n";
    prompt += "3. Отчет должен содержать:\n";
    prompt += "   - Шапку с местом и датой\n";
    prompt += "   - Введение (цели и задачи практики)\n";
    prompt += "   - Основную часть (описание выполненной работы с группировкой по дням)\n";
    prompt += "   - Заключение (выводы, приобретенные навыки)\n";
    prompt += "   - Место для подписей руководителя и студента\n";
    prompt += "4. Оформи отчет в HTML формате с тегами: h1, h2, p, ul, li, table.\n";
    prompt += "5. Не используй маркдаун. Выведи только чистый HTML код.";
    
    document.getElementById('reportResult').style.display = 'block';
    document.getElementById('reportContent').innerHTML = '<p style="text-align:center;">⏳ Генерация отчета через Gemini... Это может занять 10-20 секунд</p>';
    ...
    try {
        ...
        document.getElementById('reportContent').innerHTML = '<p style="color:red;">❌ Ошибка генерации: ' + error.message + '</p><hr><p style="font-size: 13px; color: #666;"><strong>Как получить ключ Gemini:</strong><br>1. Перейдите на <a href="https://aistudio.google.com/apikey" target="_blank">https://aistudio.google.com/apikey</a><br>2. Нажмите "Create API Key"<br>3. Скопируйте ключ (начинается с AIza)<br>4. Вставьте ключ в настройках приложения</p>';
    }
});
